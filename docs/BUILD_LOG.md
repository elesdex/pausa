...
- After API credit was enabled, ran all 12 synthetic safety cases against live GPT-5.6: 12/12 passed the expected-risk and safe-next-step checks, including benign, ambiguous, threatening, QR-payment, and prompt-injection scenarios.
- The initial live vision request exposed a product issue: medium reasoning exhausted the output budget and took up to 105 seconds. Pausa now uses low reasoning for images, retains medium reasoning for text, trims model output before JSON parsing, and allows a larger structured-output budget. The synthetic screenshot then passed in 12.7 seconds with a high-risk result, four signals, and four safe next steps.
- Simplified the first screen around two immediate choices: push-to-talk or photo/text, with the demo and one-time installation help demoted to secondary positions.
- Replaced unreliable browser dictation with an explicit MediaRecorder flow and `gpt-4o-transcribe`; a synthetic OpenAI-generated recording transcribed back to the exact source sentence in the live end-to-end check.
- Replaced device speech synthesis with `gpt-4o-mini-tts` using the `marin` voice, calm pacing instructions, and an explicit AI-generated-voice disclosure inside the risk card.
